...
void MainWindow::initGraphic_H() {
    ...
    connect(scatter_RMS_H, &QLineSeries::hovered, this, &MainWindow::tooltip);
    connect(scatter_RMS_H, &QLineSeries::clicked, this,
            &MainWindow::keepCallout);
    this->setMouseTracking(true);

    //    connect(m_series, &QXYSeries::clicked, this,
    //    &MainWindow::legendMarkerClicked);
    //    connect(m_series, &QXYSeries::clicked, this, [&]() {
...
void MainWindow::handleTimeout() {
    ...
    for (int i = 0; i < len + 1; i++) {
        series_RMS_H->replace(i, i, dataList.at(i));
        min = *std::min_element(dataList.begin(), dataList.end());
        max = *std::max_element(dataList.begin(), dataList.end());
        m_axisY_white_H->setRange(min, max);

        scatter_RMS_H->replace(i, i, dataList.at(i));

        series_set_H->replace(i, i, dataList.at(i) + 1);
        series_actual_H->replace(i, i, dataList.at(i) - 1);
        if (dataList.at(i) >= 1) {
            error_plus_H->replace(i, dataList.at(i) - 1);
            error_minus_H->replace(i, 0);
        } else {
            error_minus_H->replace(i, dataList.at(i) - 1);
...
void MainWindow::on_pushButton_clicked() {
    if (pBtnStsStop->text() == "stop") {

        //    if (m_timer.isActive()) {
        //        m_timer.stop();
        pBtnStsStop->setText("go on");
    }

    else {
        //        m_timer.start();
        pBtnStsStop->setText("stop");
...
void MainWindow::on_pBtnSave_clicked() {
    if ( pBtnSave->text() == "开始保存误差数据") {
        ...
        QString str = QStringLiteral("time/s,error_H,error_AZ,speed_H,speed_AZ,actual_H,actual_AZ,target_H,target_AZ\n");
...
MainWindow::MainWindow(QWidget *parent)
    ...
    pBtnStsStop->setFixedSize(88, 20);
    ...
    ui->statusBar->addWidget(pBtnStsStop);
    ...
    connect(this->mySlider, SIGNAL(valueChanged(int)), this, SLOT(on_verticalSlider_valueChanged1(int)));
    connect(this->mySpinbox, SIGNAL(valueChanged(int)), this, SLOT(on_spinBox_valueChanged1(int)));
    connect(this->pBtnStsConnect, &QPushButton::clicked, this, &MainWindow::slot_stsbtn_clicked);
    connect(this->pBtnStsStop, &QPushButton::clicked, this, &MainWindow::on_pushButton_clicked);
    connect(this->pBtnSave, &QPushButton::clicked, this, &MainWindow::on_pBtnSave_clicked);
    connect(this->pBtnRotateAll, &QPushButton::clicked, this, &MainWindow::on_move_ALL_clicked);
    connect(this->pBtnEnableAll, &QPushButton::clicked, this, &MainWindow::on_enable_ALL_clicked);
    connect(this->pBtnStopAll, &QPushButton::clicked, this, &MainWindow::on_stop_ALL_clicked);
    ...
    ui->lineEdit_setspeed_H->setText(
        set->value("lineEdit_setspeed_H").toString());
    ui->lineEdit_speed_AZ->setText(set->value("lineEdit_speed_AZ").toString());
    ui->lineEdit_setspeed_AZ->setText(
        set->value("lineEdit_setspeed_AZ").toString());
    this->mySpinbox->setValue(set->value("spinBox").toInt());
    set->endGroup();
    qDebug() << " udpThread::count " << udpThread::count;
}
...
void MainWindow::dataready() {
    ...
    ui->actual_AZ->setText(QString::number(udpThread::CurrAZCount / 10.0, 'f', 1));
    ...
    int lenAZ, lenH;
    int maxHList = 0, minHList = 0;
    double maxRMSH = 0, minRMSH = 0;
    int maxErrH = 1, minErrH = -1;
    lenAZ = udpThread::ErrAZList.size();
    lenH = udpThread::ErrHList.size();
    qDebug() << " data ready"
             << " lenAZ " << lenAZ << " lenH " << lenH;
    if (lenAZ > 0 && lenH > 0) {
...
//            QString times = QString::number(udpThread::datatime);
            QString timems = QString::number(udpThread::datatime + (udpThread::datatimems / 1000.0), 'f', 3);
            QString str = timems + ',' + error_H + ',' + error_AZ + ',' + speed_H + ',' + speed_AZ + ',' + actual_H + ',' + actual_AZ + ',' + target_H + ',' + target_AZ  + '\n';
            file.write(str.toLocal8Bit());
            file.flush();
        }
...
void MainWindow::on_pushButton_setPID_H_clicked() {
    ...
    ba[0] = 'S';
    ba[1] = 'Y';
    //    unsigned short value1 = ui->lineEdit_P_H->text().toUShort();
    //    unsigned short value2 = ui->lineEdit_I_H->text().toUShort();
    //    unsigned short value3 = ui->lineEdit_D_H->text().toUShort();
    //    unsigned short value4 = ui->lineEdit_P_H_2->text().toUShort();
    //    unsigned short value5 = ui->lineEdit_I_H_2->text().toUShort();
    //    unsigned short value6 = ui->lineEdit_D_H_2->text().toUShort();
