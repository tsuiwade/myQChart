...
void MainWindow::on_pBtnSave_clicked() {
  if (pBtnSave->text() == "开始保存误差数据") {
    ...
    QString str = QStringLiteral("time/"
                                 "s,RMS_H,error_H,error_AZ,speed_H,speed_AZ,"
...
void MainWindow::dataready() {
  ...
  if (bsave) {
    ...
    QString str = timems + ',' + RMS_H + ',' + error_H + ',' + error_AZ + ',' +
                  speed_H + ',' + speed_AZ + ',' + actual_H + ',' + actual_AZ +
                  ',' + target_H + ',' + target_AZ + '\n';
    file.write(str.toLocal8Bit());
    file.flush();
  }
